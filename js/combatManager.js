...
export class CombatManager {
  ...
  init() {
    ...
    this.managers.battleMaster.prepareBattle(
      this.allUnits,
      this.battleContext,
    ...
    this.managers.logManager.clear();
    this.managers.logManager.add(
      "전투 준비 완료. 시뮬레이션 시작 버튼을 누르세요.",
    );
    this.managers.logManager.flush();
  }
  ...
  async startSimulation(renderCallback) {
    ...
    if (this.managers.decorationManager) {
      await this.managers.decorationManager.applyDefaultDecorations(
        this.allUnits,
      );
    }
    ...
    for (const unit of turnOrder) {
      ...
      const enemies =
        unit.team === "player"
          ...
          : this.playerUnits.filter((u) => !u.isDead);
      const allies =
        unit.team === "player"
          ? this.playerUnits.filter((u) => !u.isDead)
          : this.enemyUnits.filter((u) => !u.isDead);
