...
export class SimulationManager {
  ...
  drawUnits(units) {
    ...
    const sorted = units.filter((u) => !u.isDead).sort((a, b) => a.y - b.y);

    sorted.forEach((unit) => {
      const drawX =
        (unit.renderX ?? unit.x) * this.CELL_SIZE + this.CELL_SIZE / 2;
      const drawY =
        (unit.renderY ?? unit.y) * this.CELL_SIZE + this.CELL_SIZE - 24;
      const spriteHeight = this.CELL_SIZE;
      const topLeftX = drawX - this.CELL_SIZE / 2;
      const topLeftY = drawY - spriteHeight;

      // draw bindings behind the unit
      const bindings = this.bindingManager.getBindings(unit);
      bindings
        .filter((b) => b.behind)
        .forEach((b) => {
      ...
      // draw bindings in front of the unit
      bindings
        .filter((b) => !b.behind)
        .forEach((b) => {
          ctx.drawImage(b.img, topLeftX + b.offsetX, topLeftY + b.offsetY);
  ...
  loadUnitSprites() {
    ...
    const promises = units.map((u) =>
      this.imageManager.load(u.image).then((img) => {
        u.sprite = img;
      }),
    );
